...
export function HistoryCard({ item, openRetryDialog }: HistoryCardProps) {
    ...
    const cancelMutation = useMutation({
        mutationFn: async (taskId: string) => {
            return await client.api.translation.cancel.$post({ json: { workflow_id: taskId } })
        },
        onSuccess: () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (taskId: string) => {
            return await client.api.history[":taskId"].$delete({ param: { taskId } })
        },
        onSuccess: () => {
        ...
        <div className="border-2 border-black dark:border-white bg-white dark:bg-zinc-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] dark:hover:shadow-[6px_6px_0px_0px_rgba(255,255,255,1)] transition-all overflow-hidden flex flex-col group/card">
            ...
            <div className="border-b-2 border-black dark:border-white bg-yellow-300 dark:bg-yellow-600 px-2 py-1.5 flex items-center justify-between gap-2 overflow-hidden">
                <div className="flex items-center gap-0 min-w-0">
                    ...
                    <div className="flex items-stretch border-2 border-black dark:border-white bg-white dark:bg-zinc-900 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,1)] overflow-hidden shrink-0">
                        ...
                        <div className={cn(
                            ...
                            statusVariant(item.status) === "destructive" ? "bg-red-500 text-white" :
                                statusVariant(item.status) === "secondary" ? "bg-blue-400 text-white" :
                                    statusVariant(item.status) === "outline" ? "bg-stone-200 dark:bg-stone-800 text-black dark:text-white" :
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                    {urls.length > 0 && (
                        unfinishedUrls.length > 0 && unfinishedUrls.length < urls.length ? (
                            ...
                            <Popover>
                                ...
                                <PopoverContent className="border-2 border-black dark:border-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] bg-white dark:bg-zinc-900 rounded-none p-0 w-auto">
                                    <div className="flex flex-col gap-0">
                                        <Link to="/" state={{ url_string: urls.join(" ") }} className="w-full">
                                        ...
                                        <Link to="/" state={{ url_string: unfinishedUrls.join(" ") }} className="w-full">
                                            <div className="px-2 py-1.5 hover:bg-blue-200 dark:hover:bg-blue-900 font-mono font-bold uppercase text-xs cursor-pointer border-b border-gray-200 dark:border-zinc-700 last:border-b-0">
                                                View Unfinished
                                            </div>
                                        </Link>
                            ...
                            <ViewButton
                                onClick={() => navigate("/", { state: { url_string: urls.join(" ") } })} />
                        )
                    )}
                    ...
                    {canRetry && (
                        unfinishedUrls.length > 0 && unfinishedUrls.length < urls.length ? (
                            <Popover>
                                ...
                                <PopoverContent className="border-2 border-black dark:border-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] bg-white dark:bg-zinc-900 rounded-none p-0 w-auto">
                                    <div className="flex flex-col gap-0">
                                        <button onClick={() => openRetryDialog(urls)} className="px-2 py-1.5 hover:bg-yellow-200 dark:hover:bg-yellow-900 font-mono font-bold uppercase text-xs cursor-pointer border-b border-gray-200 dark:border-zinc-700 last:border-b-0 text-left bg-transparent">
                                            Retry All
                                        </button>
                                        <button onClick={() => openRetryDialog(unfinishedUrls)} className="px-2 py-1.5 hover:bg-yellow-200 dark:hover:bg-yellow-900 font-mono font-bold uppercase text-xs cursor-pointer border-b border-gray-200 dark:border-zinc-700 last:border-b-0 text-left bg-transparent">
                                            Retry Unfinished
                                        </button>
                                    </div>
                                </PopoverContent>
                        ...
                        <AlertDialogContent className="border-2 border-black dark:border-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] bg-white dark:bg-zinc-900 rounded-none">
                            <AlertDialogHeader>
                                <AlertDialogTitle className="font-black uppercase text-2xl tracking-tighter">
                                    {isInProgress ? "Cancel Workflow?" : "Delete Item?"}
                                </AlertDialogTitle>
                                <AlertDialogDescription className="font-mono font-bold text-black dark:text-white border-l-4 border-red-500 pl-2">
                                    {isInProgress ? "STOP THE CURRENT PROCESS." : "THIS ACTION CANNOT BE UNDONE."}
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter className="gap-2">
                                <AlertDialogCancel className="border-2 border-black dark:border-white rounded-none font-bold uppercase">Cancel</AlertDialogCancel>
                                <AlertDialogAction
                                    onClick={() => isInProgress ? cancelMutation.mutate(item.taskId) : deleteMutation.mutate(item.taskId)}
                                >
            ...
            <div className="p-4 flex-1 space-y-4">
                <div className="flex flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-white dark:bg-zinc-800 border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,1)] text-xs font-black font-mono">
                        ...
                        <span>{item.total > 0 ? `${item.current}/${item.total}` : urls.length} <span className="text-[10px] opacity-60 uppercase">Files</span></span>
                    </div>
                    {/* API Key Name */}
                    {item.apiKeyName && (
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-blue-400 text-white border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,1)] text-xs font-black font-mono uppercase">
                            <span className="text-[10px] opacity-70 mr-1.5">Key:</span>
                    ...
                    {(item.provider || item.model) && (
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-stone-100 dark:bg-zinc-800 border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,1)] text-xs font-black font-mono">
                            <span className="break-all uppercase">
                                <span className="text-[10px] opacity-60 mr-1.5">Model:</span>
            ...
            {isInProgress && (
                ...
                >
                    <div className="absolute inset-0 flex items-center justify-end px-1.5 pointer-events-none">
                        <span className="text-[9px] font-black text-black drop-shadow-sm">
            ...
            {item.errorMessage && (
                <div className="px-4 py-3 bg-red-100 dark:bg-red-900 border-t-2 border-black dark:border-white text-xs font-bold font-mono text-black dark:text-white flex items-start gap-2">
                    ...
                    <div className="flex-1">
                        <span className="bg-red-600 text-white px-1.5 font-black mr-2 uppercase">System Error</span>
                        <span className="leading-tight break-all">{item.errorMessage}</span>
                    </div>
                </div>
            )}
        </div>
...
function DeleteButton({ onClick, isInProgress, ...props }: React.ComponentProps<"button"> & { isInProgress?: boolean }) {
    return (
        <Button size="xs" {...props} className="bg-white dark:bg-zinc-900 dark:text-red-400 dark:hover:bg-red-950 text-red-600 hover:bg-red-100" onClick={onClick}>
            {isInProgress ? (
...
function RetryButton({ onClick, ...props }: React.ComponentProps<"button">) {
    return (
        <Button size="xs" {...props} className="bg-white dark:bg-zinc-900 text-black dark:text-white hover:bg-yellow-200 dark:hover:bg-yellow-900 border-2 border-black dark:border-white" onClick={onClick}>
            <RotateCcw className="h-3.5 w-3.5" />
        </Button>
    );
}
...
function ViewButton({ onClick, ...props }: React.ComponentProps<"button">) {
    return (
        <Button size="xs" {...props} className="bg-white dark:bg-zinc-900 text-black dark:text-white hover:bg-blue-200 dark:hover:bg-blue-900" onClick={onClick}>
