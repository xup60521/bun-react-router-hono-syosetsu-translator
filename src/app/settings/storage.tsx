...
export default function StoragePanel(): React.JSX.Element {
        ...
        <div className="w-full max-w-4xl mx-auto space-y-8 pb-8">
            ...
            <div className="space-y-4">
                <div className="border-b-4 border-black dark:border-white pb-4">
                    <h3 className="text-2xl sm:text-3xl font-black uppercase tracking-tighter">Google Drive</h3>
                ...
                {isPending ? (
                    ...
                    <div className="border-2 border-black dark:border-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] bg-white dark:bg-zinc-900 p-4 flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <div className="flex gap-2 self-end sm:self-auto">
                            ...
                            >
                                ...
                                <span className="sm:hidden inline">Log out</span>
                            </Button>
                        </div>
                    </div>
                ) : (
                    <div className="border-2 border-black dark:border-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] bg-gray-50 dark:bg-zinc-900 p-8 flex flex-col items-center justify-center gap-4 text-center">
                        ...
                        <h4 className="text-xl font-bold font-mono text-black dark:text-white">Not Connected</h4>
                        <p className="text-gray-500 dark:text-gray-400 max-w-sm">Connect your Google Drive account to select folders for saving translated novels.</p>
                        <Button onClick={handleGoogleLogin} className="w-full max-w-xs h-12 text-lg border-2 border-black dark:border-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] bg-white dark:bg-zinc-800 text-black dark:text-white hover:bg-yellow-300 dark:hover:bg-yellow-700">
                            Connect with Google
                        </Button>
                    </div>
                )}
...
function SelectFolder(): React.JSX.Element {
    const [openPicker, setOpenPicker] = useAtom(openPickerAtom);
    const { data: selectedFolder, isPending } = useQuery({
    ...
    return (
        <div className="space-y-4 pt-4 border-t-4 border-black dark:border-white border-dashed">
            <div>
                <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tighter">Output Path</h3>
            ...
            <div className="space-y-4">
                <Label className="text-sm font-bold uppercase font-mono bg-yellow-300 dark:bg-yellow-600 w-fit px-1 border-2 border-black dark:border-white">Selected Folder</Label>
...
function GoogleDrivePicker() {
    ...
    return (
        <DrivePicker
            client-id={process.env.VITE_GOOGLE_CLIENT_ID!}
            app-id={process.env.VITE_GOOGLE_APP_ID!}
