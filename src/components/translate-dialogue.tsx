...
function TranlationForm({
    ...
    const modelListQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!provider) return [];
            const models = await client.api.models[":provider"].$get({ param: { provider } })
            // const models = await get_modelList({ data: provider });
    ...
    // const translate = useServerFn(translate_serverfn);
    const submitTranslationMutation = useMutation({
        mutationKey: ["send-translation-request"],
        mutationFn: async (
            payload: FormData & { urls: string[]; folder_id: string },
        ) => {
            const response = await client.api.translation.translate.$post({ json: payload })
            // const response = await translate({ data: payload });
            return response;
        },
    });
    ...
    const onSubmit = async (translationSettings: FormData) => {
        ...
        // console.log(payload);
        try {
            await submitTranslationMutation.mutateAsync(payload);
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="flex flex-col gap-4">
                ...
                <Field data-invalid={!!form.formState.errors.encrypted_api_key}>
                    ...
                    <FieldError errors={[form.formState.errors.encrypted_api_key]} />
                </Field>
                ...
                <Field data-invalid={!!form.formState.errors.concurrency}>
                    ...
                    <Input
                        ...
                        onChange={(e) =>
                            form.setValue("concurrency", parseInt(e.target.value, 10))
                        }
                    />
                ...
                <Field data-invalid={!!form.formState.errors.batch_size}>
                    ...
                    <Input
                        ...
                        onChange={(e) =>
                            form.setValue("batch_size", parseInt(e.target.value, 10))
                        }
                    />
